...
Route::group(
    ...
    Route::get('/teacher/dashboard', function () {
        ...
        // elquent طريقة1
        // id مشان نجيب الطلاب تبع ل قسم
        $ids = Teacher::findorFail(auth()->user()->id)->Sections()->pluck('section_id');  // (auth()->user() is teacher  يلي  فاتح
        $data['count_sections']= $ids->count();
        $data['count_students']= \App\Models\Student::whereIn('section_id',$ids)->count();  // القسم الذي يخص المعلم فقط

        // query builder طريقة2
