<div>
    <div>
        <div class="card card-statistics mb-30">
            <div class="card-body">
              <h5 class="card-title"> {{$data[$counter]->title}}</h5>    <!-- هنا الكاونتر مشان يعرض كل سؤال لحالو  -->
                ...
                @foreach(preg_split('/(-)/', $data[$counter]->answers) as $index=>$answer)
                    ...
                    <!-- wire:click="nextQuestion({{$data[$counter]->id}},
                    روح على الايفواير على الكلاس و شوف الفانكشن وخود معك الاشياء الخاصة بكل سؤال
                    من جدول الكوستشن
                     -->
                @endforeach
