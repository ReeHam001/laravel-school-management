...
class AddParent extends Component
{
    ...
    public $catchError,$updateMode = false,
               $photos,
               $show_table = true,  // $show_table  لعرض الجدول اولا
               $Parent_id;

    public $currentStep = 1,  // open step number one first

    // you have to declair all the var in blade you wandt to use here
    ...
    public function render()  // like index
    {
    ...
    public function updated($propertyName)  // real time validation -> put @error in blade to show validation error
    {
    ...
    public function showformadd(){  // مشان يعرض الاضافة بدل الجدول
    ...
    //firstStepSubmit
    public function firstStepSubmit()  // go to 2 tab  رابط زر التالي باول خطوة
    {
       $this->validate([  // يتحقق بتعبئة جميع الحقول المطلوبة
    ...
    //secondStepSubmit
    public function secondStepSubmit() // go to 3 tab
    {
        ...
        $this->validate([ // يتحقق بتعبئة جميع الحقول المطلوبة
            ...
            'Passport_ID_Mother' => 'required|unique:my__parents,Passport_ID_Mother,' . $this->id,  // unique بجدول my__parents
    ...
    public function submitForm(){
        ...
        try {
            ...
            // Father_INPUTS
            $My_Parent->Email = $this->Email;  // this refer to request
            ...
            $My_Parent->save();  // saved father and mother

            if(!empty($this->photos)){
                foreach($this->photos as $photo) {  // folder with father id   //  $disk got to config -> file system -> disks
                    $photo->storeAs($this->National_ID_Father, $photo->getClientOriginalName(), $disk = 'parent_attachments');
                    ParentAttachment::create([
                        'file_name' => $photo->getClientOriginalName(),  // اخر اي دي دخل ممكن اذا 2 بنفس اللحظة يحصل مشكلة لكن احتمال قليل
                        'parent_id' => My_Parent::latest()->first()->id,  // or we use relationship  فرق جزء من الثواني بكون اول عملها
                    ]);
                }
            }

            $this->successMessage = trans('messages.success');
            $this->clearForm();  // مشان يفضي الفورم
            $this->currentStep = 1;
        }
    ...
    {
        $this->show_table = false; // اخفاء الجدول
        $this->updateMode = true;  // تفعيل واجهة التعديل

        $My_Parent = My_Parent::where('id',$id)->first();   // جاب البيانات

        // تعبئة حقول الفورم تبع الابديت
        $this->Parent_id = $id;
    ...
    //firstStepSubmit
    public function firstStepSubmit_edit() // edit تفعيل
    {
    ...
    //back
    public function back($step)  // رجوع خطوة لورا
    {
