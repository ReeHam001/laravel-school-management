...
class FeeInvoicesRepository implements FeeInvoicesRepositoryInterface
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            foreach ($List_Fees as $List_Fee) {
                ...
                $Fees->Classroom_id = $request->Classroom_id;;  // لانه ثابتين للطالب
                ...
                $StudentAccount->type = 'invoice';              // فاتورة - سند قبض - صرف
                $StudentAccount->fee_invoice_id = $Fees->id;
                $StudentAccount->student_id = $List_Fee['student_id'];
                $StudentAccount->Debit = $List_Fee['amount'];  // اضافة ادي في رسوم عالطالب
                $StudentAccount->credit = 0.00;  // لسه مادفع شي
    ...
    public function destroy($request)
    {
        try
         {
            Fee_invoice::destroy($request->id);  // on delete cascade with sutdent account table
